...
export function findQuestion(
    ...
    const newQuestion = questions.find((q: Question): boolean => q.id === id);
...
export function sumPublishedPoints(questions: Question[]): number {
    const questionSum = questions.reduce(
        (sum: number, current: Question) =>
            current.published === true ? sum + current.points : sum,
        0
    );
    return questionSum;
}
...
export function toCSV(questions: Question[]): string {
    const stringRep = questions.reduce(
        (strings: string[], q: Question) => [
            ...strings,
            q.id +
                "," +
                q.name +
...
export function sameType(questions: Question[]): boolean {
    ...
    if (questions.length === 0) {
        ...
        result = questions.reduce(
            (res: boolean, q: Question) =>
                q.type === qType && res === true ? res : (res = false),
            true
        );
        return result;
    }
